...
#从句子级转化为文档级的位置关系，为train，validate，test分别创建pos，ner，char，word的npy矩阵文件存储信息方便后面取用和训练
...
def init(data_file_name, rel2id, max_length = 16384, suffix=''):
    ...
    print(os.path.join(out_path, name_prefix + suffix + '.json'))
		...
		for j, word in enumerate(words):
			...
			if j < max_length:
				...
				for c_idx, k in enumerate(list(word)):
					if c_idx>=char_limit: #No character past the char_limit is recorded
						break
